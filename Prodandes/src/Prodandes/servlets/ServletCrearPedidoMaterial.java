...
public class ServletCrearPedidoMaterial extends HttpServlet
{
	...
	private void procesarSolicitud( HttpServletRequest request, HttpServletResponse response ) throws IOException
	{
		...
		int pIdProveedor = Integer.parseInt(request.getParameter("id"));
		String pMaterial = request.getParameter("material");
		int pCantidad = Integer.parseInt(request.getParameter("volumen"));
		int pTiempo = Integer.parseInt(request.getParameter("tiempo"));
		int pCosto = Integer.parseInt(request.getParameter("costo"));

		PedidoMaterial resp = null;
		try {
			resp = joda.crearPedido(pIdProveedor, pMaterial, pCantidad, pTiempo, pCosto);
	 ...
	 */
	private void imprimirEncabezado( HttpServletResponse response, PedidoMaterial pedido ) throws IOException
	{
		...
		out.println("<h3><FONT SIZE=8>Pedido</font></h3>");
		out.println("<p>Se genero el pedido correctamente. </p>");
		out.println("</div>");
		out.println("    <label for=\"categoria2\"><FONT SIZE=8><strong> Datos del pedido </strong></font></label> ");
		out.println("  </p>");
		out.println("    <label for=\"categoria2\"> Id Pedido: "+ pedido.darId() + "</label> ");
		out.println("  </p>");
		out.println("    <label for=\"categoria2\"> Identificacion Proveedor: "+ pedido.darIdProveedor() + "</label> ");
		out.println("  </p>");
		out.println("    <label for=\"categoria2\"> Material: "+ pedido.darMaterial() + "</label> ");
		out.println("  </p>");
		out.println("    <label for=\"categoria2\" > Volumen: "+ pedido.darCantidad() + "</label> ");
		out.println("  </p>");
		out.println("    <label for=\"categoria2\"> Tiempo de entrega: "+ pedido.darTiempo() + " semanas</label> ");
		out.println("  </p>");	
		out.println("    <label for=\"categoria2\"> Costo: "+ pedido.darCosto() + "</label> ");
		out.println("  </p>");
		out.println("  <p>&nbsp;</p>");
		out.println("  <p>&nbsp;</p>");
		out.println("<p>&nbsp;</p>");
