...
public class ServletBuscarEmpleadoProductivo 
extends HttpServlet
{
	...
	private void imprimirEncabezado( HttpServletResponse response ) throws IOException
	{
		...
		out.println("<h3><FONT SIZE=5>Buscar Empleado Productivo</font></h3>");
		out.println("<p>Es el operario que ha realizado el mayor número de operaciones de una cierta etapa de producción o de una cantidad específica. Note que en la respuesta puede haber más de un operario.</p>");
		out.println("</div>");
		out.println("<form id=\"form1\" name=\"form1\" method=\"post\" action=\"buscarOperarioProductivoRes.htm\">");
		out.println("  <p>");
		out.println("    <label for=\"material\"><FONT SIZE=7>Etapa:</font></label> ");
