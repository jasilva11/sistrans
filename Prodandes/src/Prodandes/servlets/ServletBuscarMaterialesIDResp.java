...
public class ServletBuscarMaterialesIDResp extends HttpServlet
{
	...
	private void procesarSolicitud( HttpServletRequest request, HttpServletResponse response ) throws IOException
	{
		...
		int id = Integer.parseInt(request.getParameter("material"));

		ArrayList resp = new ArrayList();
		try {
			resp = joda.conMateriales2(id);
	...
	private void imprimirEncabezado( HttpServletResponse response, ArrayList resultados ) throws IOException
	{
		...
		for (int i = 0; i < resultados.size(); i++) 
		{
			...
			{
				MaterialPedido x = (MaterialPedido) resultados.get(i);
				int y = i+1;
				...
				out.println("  </p>");
				out.println("    <label for=\"categoria2\"> ID Pedido: "+ x.darIdPedido() + "</label> ");
				out.println("  </p>");
				out.println("    <label for=\"categoria2\"> Tipo: "+ x.darTipo() + "</label> ");
				out.println("  </p>");
				out.println("    <label for=\"categoria2\"> Costo: "+ x.darCosto() + "</label> ");
				out.println("  </p>");
				out.println("    <label for=\"categoria2\" > Volumen: "+ x.darVolumen() + "</label> ");
				out.println("  </p>");
				if(x.darEtapa() != 0)
				{
					out.println("    <label for=\"categoria2\"> Esta en etapa: Si</label> ");
					out.println("  </p>");
					out.println("    <label for=\"categoria2\"> ID Etapa: "+ x.darEtapa() + "</label> ");
					out.println("  </p>");
				...
				{
					out.println("    <label for=\"categoria2\"> Esta en etapa: No</label> ");
					out.println("  </p>");
					out.println("    <label for=\"categoria2\"> Fecha Inicio: "+ x.darInicio() + "</label> ");
					out.println("  </p>");
