...
public class ServletBuscarEmpleadoProductivoRes extends HttpServlet
{
	...
	private void procesarSolicitud( HttpServletRequest request, HttpServletResponse response ) throws IOException
	{
		...
		String etapa = request.getParameter("material");

		ArrayList resp = new ArrayList();
		try {
			resp = joda.conOperarios(etapa);
	...
	private void imprimirEncabezado( HttpServletResponse response, ArrayList resultados ) throws IOException
	{
		...
		out.println("<h3><FONT SIZE=8>Operario mas productivo</font></h3>");
		out.println("</div>");
		for (int i = 0; i < resultados.size(); i++) 
		{
			if (resultados.get(i) != null)
			{
				Operario x = (Operario) resultados.get(i);
				out.println("    <label for=\"categoria2\"> ID Empleado: "+ x.darId() + "</label> ");
				out.println("  </p>");
				out.println("    <label for=\"categoria2\"> Cargo: "+ x.darCargo() + "</label> ");
				out.println("  </p>");
				out.println("    <label for=\"categoria2\"> Cuenta bancaria: "+ x.darCuenta() + "</label> ");
				out.println("  </p>");
				out.println("    <label for=\"categoria2\"> Duracion Contrato: "+ x.darDuracion() + "</label> ");
				out.println("  </p>");
				out.println("    <label for=\"categoria2\"> Etapa de produccion: "+ x.darArea() + "</label> ");
				out.println("  </p>");	
			}
		}
		out.println("<p>&nbsp;</p>");
