...
public class ServletBuscarMaterialesResp extends HttpServlet
{
	...
	private void procesarSolicitud( HttpServletRequest request, HttpServletResponse response ) throws IOException
	{
		...
		String parametro = request.getParameter("material");
		...
		try {
			resp = joda.conMateriales(parametro,parametro2, tipo);
	...
	private void imprimirEncabezado( HttpServletResponse response, ArrayList resultados ) throws IOException
	{
		...
		for (int i = 0; i < resultados.size(); i++) 
		{
			if (resultados.get(i) != null)
			{
				...
				out.println("  </p>");
				out.println("    <label for=\"categoria2\"> Compone: "+ x.darCompone() + "</label> ");
				out.println("  </p>");
				...
				out.println("  </p>");
				out.println("    <label for=\"categoria2\"> Etapa: "+ x.darEtapa() + "</label> ");
				out.println("  </p>");
